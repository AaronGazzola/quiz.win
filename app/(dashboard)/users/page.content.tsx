...
export function UsersPageContent() {
  ...
  const handleRowClick = (userData: UserWithDetails) => {
    ...
    if (selectedOrganizationIds && selectedOrganizationIds.length > 0 && userData.member) {
      userData.member.forEach(memberItem => {
        if (selectedOrganizationIds.includes(memberItem.organizationId)) {
          sharedOrganizations.push({
            organizationId: memberItem.organizationId,
            organizationName: memberItem.organization.name,
            currentRole: memberItem.role,
            newRole: memberItem.role
          });
        }
      });
    }
                ...
                users.map((userData) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-muted-foreground">
                      {getOrganizationsDisplay(userData.member || [])}
                    </td>
