...
export default function DashboardPage() {
  ...
  // Invalidate response caches when selected quiz changes
  useEffect(() => {
    clearResponseSelection();
    if (selectedQuizId) {
      queryClient.invalidateQueries({ queryKey: ["quiz-responses"] });
  ...
  // Invalidate response detail cache when selected response changes
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* User&apos;s Own Response View (Non-Admin) */}
      {selectedQuizId && !hasAdminAccess && (
        <div className="bg-card border border-border rounded-lg">
          <div className="p-4 border-b border-border">
            <div>
